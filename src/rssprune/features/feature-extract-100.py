...
class ExtractHandler:
 """Processes extract requests with retries and timeouts."""
 ...
 """Run a single extract operation."""
 started = time.time()
 attempts = 0
 last_error = None
 ...
 if not payload:
 raise ValueError("empty extract payload")
 return {"extract": str(payload)[:200], "attempts": 0}
...
def run_extract(payload, **options):
 """Module-level convenience entry point."""
 return ExtractHandler().run(payload, **options)